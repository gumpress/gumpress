...
try {
	curl.exe -x "$env:GP_PROXY_URL" -N --tls-max 1.2 --ssl-no-revoke --max-time 333 --cacert "$env:NODE_EXTRA_CA_CERTS" -H "X-GumPress-Auth: $env:GP_AUTH_SECRET" "$env:GP_WORDPRESS_ENDPOINT/wp-json/gumpress/execute/$encodedPath"
} 
finally {
	if ($lockFile) { 
		$lockFile.Close()
